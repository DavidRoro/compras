...
if (!empty($_POST['agregar'])) {
    $consulta = db_query("select * from notaremisionventa where remv_estado='PENDIENTE' and usu_id=$idUsu");
    if (mysqli_num_rows($consulta) > 0) {
        $consucabe = db_query("select * from notaremisionventa where remv_estado='PENDIENTE' and usu_id=$idUsu");
        $ID = mysqli_fetch_array($consucabe);
        $ped = $ID[0];
        $cod = $_POST['txtid'];
        $cant = $_POST['txtcantidad'];
        //insertar detalle
        $detalle = db_query(" call sp_detnotaremisionventa($ped,$cod,$cant)");

        if ($detalle) {
            echo "<script>location.href='notaremisionventa_agregar.php'</script>";
//            echo "<div class='alert alert-success'>INSERTO EXITOSAMENTE</div>";
//            header("refresh:3;url=notaremision_agregar.php");
        } else {
            echo "<script>alert('YA INSERTO TODOS LOS REGISTROS');location.href='notaremisionventa_agregar.php'</script>";
        }
    } else {
        $fecha = date('Y-m-d');
...
//        $cabecera = db_query("insert into pedido(ped_fecha,ped_estado,suc_id,usu_id) values('$fecha','PENDIENTE',$sucursales,$idUsu)");
        $cabecera = db_query(" call sp_notaremisionventa('$fecha','$inicio','$fin','$motivo',$vehiculo,,'PENDIENTE'$per,$idUsu,$cli)");
        //consultar cabecera
        $consucabe = db_query("select * from nota_remisionventa where remv_estado='PENDIENTE' and usu_id=$idUsu");
        $ID = mysqli_fetch_array($consucabe);
        $ped = $ID[0];
        $cod = $_POST['txtid'];
        $cant = $_POST['txtcantidad'];
...
if (!empty($_GET['imprimir'])) {
    $f1 = $_GET['vcod'];
    $impre = db_query("update nota_remisionventa set remv_estado='GENERADO' where idnotaremisionventa=$f1");
    if ($impre) {
        echo "<script>location.href='notaremisionventa_impresion.php?vcod=$f1'</script>";
    }
